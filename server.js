// Seixal • Gasolina 95 proxy (DGEG HTML scraper) — v2 FINAL
// Lê as páginas oficiais da DGEG e extrai Gasolina 95 (Simples e Especial).
// Aplica -€0.19 na Galp. CORS ON. Cache 15 min.
...
function extractGasolina(html) {
  const out = [];
  const rxBlock = /Gasolina\s*95[^€]*€\s*([0-9],[0-9]{3})/gi;
  let m;
  while ((m = rxBlock.exec(html)) !== null) {
    const val = parsePrice(m[1]);
    if (isFinite(val)) {
      const block = m[0].toLowerCase();
      const tipo = block.includes("especial") || block.includes("aditiv") || block.includes("+")
        ? "especial" : "simples";
...
async function fetchStationHTML(id) {
  ...
  try {
    ...
    console.error("Erro fetch HTML:", id, e);
    return "";
  }
}
